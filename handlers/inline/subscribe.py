...
from utils.database import subscribe, unsubscribe, is_subscription_exists
...
async def sub_inline_callback(query: CallbackQuery, callback_data: SubscriptionCallback):
    ...
    if await is_subscription_exists(query.from_user.id, article):
        ...
        await subscribe(query.from_user.id, article)
...
async def unsub_inline_callback(query: CallbackQuery, callback_data: UnsubscriptionCallback):
    ...
    if await is_subscription_exists(query.from_user.id, article):
        ...
        await unsubscribe(query.from_user.id, article)
    else:
